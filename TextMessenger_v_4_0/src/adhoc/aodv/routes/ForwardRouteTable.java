...
public class ForwardRouteTable {
	...
	public void setValid(int destinationAddress, int destinationSeqNumber, boolean validValue) throws NoSuchRouteException {
		synchronized (tableLock) {
			ForwardRouteEntry entry = entries.get(destinationAddress);
			if(entry != null) {
				// Dr. Byun - If the forward route becomes "invalid", remove it immediately from the table.
				if (validValue == false)
				{
					removeEntry(destinationAddress);
					return;
				}
				...
				// Dr. Byun
				// Log.i("RouteTable", destinationAddress + " is setValid()");
	...
	/**
	 * Dr. Byun: Only used for testing purpose at "ContactsView.java" to display next hop information
	 */
	public int getNextHop(int destination) {
		synchronized (tableLock) {
			for(ForwardRouteEntry f :entries.values()){
				if(f.getDestinationAddress() == destination)
				return (f.getNextHop());
			}	
