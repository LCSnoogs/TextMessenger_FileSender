...
public class RouteTableManager {
	...
	protected boolean createRouteRequestEntry(RREQ rreq, boolean setTimer) {
		...
		if (routeRequestTable.addRouteRequestEntry(entry, setTimer)) {
			// Dr. Byun
		    Log.i("BYUN", "RouteTableManager.java - invoke addRouteRequestEntry() - 1");
	...
	// Dr. Byun - Change to "public" to get the forward route information at "ContactsView.java". - Debugging purpose.
	...
	private class TimeoutNotifier extends Thread {
		...
		public void run() {
			while (keepRunning) {
				// Dr. Byun
			    // Log.i("RouteTableManager", "TimeoutNotifier: " + forwardRouteTable.toString());
			    // Log.i("RouteTableManager", " ");
			    // Log.i("RouteTableManager", "TimeoutNotifier: " + routeRequestTable.toString());
							...
							if (route.getSourceAddress() == nodeAddress) {
								if (!validForwardRouteExists(route.getDestinationAddress(), route.getDestinationSequenceNumber())) {
									if (route.resend()) {
										...
										// Dr. Byun
									    Log.i("BYUN", "RouteTableManager.java - invoke addRouteRequestEntry() - 2");
