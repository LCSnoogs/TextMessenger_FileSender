...
import android.TextMessenger.view.ContactsView;   // For no wireless signal simulation
...
public class Receiver implements Runnable {
			...
			while (keepRunning) {
				try {
					...
					if(msg.senderNodeAddress != nodeAddress) {
						// To simulate no wireless signal
						if(((nodeAddress == 1) && (msg.senderNodeAddress == 2) && (ContactsView.simulateValue == true)) ||
						   ((nodeAddress == 2) && (msg.senderNodeAddress == 1) && (ContactsView.simulateValue == true)))
						{	
						    Log.i("Receiver", "Drop the signal...");  
						    ; // Drop the signal
						}
						else
						{
	...
	private void helloMessageReceived(HelloPacket hello) {
		try {
			...
			// Dr. Byun
			Log.i("RouteTable", "================================================================");
			Log.i("RouteTable", hello.getSourceAddress() + " has no such route exception.");
			Log.i("RouteTable", "================================================================");
			...
			routeTableManager.createForwardRouteEntry(	hello.getSourceAddress(),
														...
														1,	true);
			Debug.print("Receiver: catch: " + hello.getSourceAddress());
		}
	...
	private void routeReplyReceived(RREP rrep, int senderNodeAddress) {
		...
		try {
			...
			//FIXME 
			Debug.print("Receiver: FATAL ERROR - RouteNotValidException");
			try {
	...
	private void routeRequestReceived(RREQ rreq, int senderNodeAddress) {
		...
		// Dr. Byun
	    Log.i("CreateRouteRequest", "Receiver.java invokes createRouteRequestEntry()");
	...
	private void userDataPacketReceived(UserDataPacket userData) {
		if (userData.getDestinationAddress() == nodeAddress 
	    	...
	    	// Dr. Byun 
			Log.i("Receiver", "userDataPacketReceived() - " + userData.getData());
		} else {
			sender.queueUserMessageToForward(userData);
	    	// Dr. Byun
			Log.i("Receiver", "userDataPacketReceived() to forward - " + userData.getData());
		}
		Debug.print("Receiver TXT: received User Data Packet!!!");
	}
